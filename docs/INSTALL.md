...
Setpiece v0.5.0+ is **notarized by Apple** — no Gatekeeper bypass, no quarantine strip, no "cannot be verified" prompt. Double-click to install.
...
1. **Download** the latest `Scene-X.Y.Z.dmg` from the [Releases page](https://github.com/ChiFungHillmanChan/setpiece/releases).
2. **Double-click** the DMG to mount it.
3. **Drag `Scene.app` into `Applications`**.
4. Eject the mounted DMG.
5. Open Setpiece from Launchpad or `/Applications`.

No right-click, no `xattr`, no System Settings detour. Because the DMG is notarized, macOS trusts it on first launch.

## Grant Accessibility permission

Setpiece needs Accessibility access to move other apps' windows.

1. Launch Setpiece. The Setpiece menu bar icon appears at the top-right (a small `⌧` rectangle-group icon).
...
4. Find **Setpiece** in the list and toggle it **ON**.
5. Return to Setpiece — within 2 seconds the menu updates to show all layout presets.
...
3. Remove Setpiece from the list (select, press `−`).
...
1. Quit Setpiece from its menu (`Quit Setpiece`).
2. Drag `/Applications/Scene.app` to the Trash.
3. Optional: remove Setpiece from **System Settings → Privacy & Security → Accessibility** (click Setpiece, then the `−` button).
...
| Post-upgrade: grant looks ON but Setpiece says it's off | See "Upgrading from v0.4.3 or earlier" above. This is a one-time transition. |
| Setpiece won't resize windows | Some apps (Apple System Settings, older Preferences apps) enforce a minimum size and ignore AX sizing requests. Not a Setpiece bug. |
