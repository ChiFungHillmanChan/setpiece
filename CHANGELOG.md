...
- **Every Settings toolbar button is back on macOS 26 (Tahoe).** On Layouts, the **+**, **+ Custom**, delete, reset-to-seed and **Restore Default Presets** buttons were all missing; on Workspaces, **New**, **Duplicate** and **Delete** were missing. With no toolbar, layouts and workspaces could be edited but never created, duplicated, or deleted — the entire create/delete surface of the app was unreachable from Settings, and the window looked unfinished. Reported as [#4](https://github.com/ChiFungHillmanChan/scene-macos/issues/4).
- **Root cause: the translucency backdrop broke the toolbar bridge.** V0.7.2 added whole-window Liquid Glass by making an `NSVisualEffectView` container view controller the window's `contentViewController` and nesting the SwiftUI `NSHostingController` inside it as a child. `NSHostingController` only publishes its SwiftUI `.toolbar` content into `view.window.toolbar` when it *is* the window's content view controller; as a child it publishes nothing, silently and with no warning. The window therefore had no `NSToolbar` at all — `window.toolbar` was `nil`, not merely empty — so nothing the tabs declared could render. Confirmed by isolating all four combinations of (wrapped vs. root hosting controller) × (Tahoe chrome vs. plain chrome): only the two wrapped cases produced a `nil` toolbar, and the transparent-title-bar chrome was innocent.
- **Fix: the backdrop moved into SwiftUI.** The `NSHostingController` is once again the window's root `contentViewController`, and the `.underWindowBackground` material is applied as a SwiftUI `background` layer via a small `NSViewRepresentable`. The Liquid Glass appearance is unchanged — floating glass sidebar, transparent title bar, whole-window blur — and the toolbar bridge is intact. The two-branch window construction collapses into one path shared by every macOS version.
- **Scope.** macOS 14 and 15 were never affected; they took the untouched `else` branch. Affected releases were v0.7.2, v0.7.3 and v0.7.4 on macOS 26 only.
- **Tests: 394/394**, unchanged — the defect is in AppKit/SwiftUI window plumbing inside SceneApp, which `swift test` (SceneCore only) cannot reach. Verified instead against the running app: the Layouts tab's toolbar reports `Add, square.on.square.squareshape.controlhandles, Bin, arrow.counterclockwise, Restore Default Presets` and Workspaces reports `New, Duplicate, Bin`.
