...
Setpiece runs **unsandboxed** and requires the macOS **Accessibility (AX)** permission in
...
Only the latest release receives security fixes. Setpiece ships an in-app updater, so
users on older builds are prompted to update.

| Version | Supported |
| ------- | --------- |
| Latest release ([releases](https://github.com/ChiFungHillmanChan/setpiece/releases/latest)) | ✅ |
| Any earlier version | ❌ |
...
Preferred: [**Report a vulnerability**](https://github.com/ChiFungHillmanChan/setpiece/security/advisories/new)
via GitHub's private security advisories.

Alternative, if you cannot use GitHub advisories: `hillmanchan709@gmail.com` with
`[Setpiece security]` in the subject.

Please include the Setpiece version, macOS version, chip (Apple Silicon / Intel), and the
...
Setpiece is maintained by one person in Hong Kong (UTC+8), unfunded. These are honest
targets, not an SLA:
...
There is no bug bounty — Setpiece has no funding.
...
Setpiece holds an AX grant, which is effectively full UI control of the machine. Anything
that lets an untrusted input reach the AX layer, or that causes Setpiece to act on a window
...
Setpiece downloads a DMG, verifies the `codesign` Team Identifier, then hands off to a
...
compromised update inherits Setpiece's existing Accessibility grant.
...
- The Accessibility permission requirement itself. Setpiece cannot move other apps' windows
  without it; this is a documented design constraint, not a vulnerability.
- The app being unsandboxed. Same reason — App Sandbox forbids the AX APIs Setpiece needs.
...
For context when assessing a report, Setpiece already does the following:
